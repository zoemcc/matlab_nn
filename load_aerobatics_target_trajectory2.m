function [x_init, x_target, u_target] = load_aerobatics_target_trajectory2(dt)
...
T_hover_at_start = 20;
T_half_sideways_roll_at_start = 20;
...
for k=1:T_half_sideways_roll_at_start
	x_target(idx.ned,T0+k) = [0;0;0];
	x_target(idx.ned_dot,T0+k) = [0;0;0];
	x_target(idx.pqr,T0+k) = [0;0;0];
	x_target(idx.axis_angle,T0+k) = [0;0;0];

	x_target(idx.pqr(1),T0+k) = pi/(T_half_sideways_roll_at_start*dt);
	x_target(idx.axis_angle(1),T0+k) = pi*k/T_half_sideways_roll_at_start;
	
end
T0 = T0 + T_half_sideways_roll_at_start;
...
x_inverted_hover_at_start(idx.axis_angle) = [pi;0;0];
